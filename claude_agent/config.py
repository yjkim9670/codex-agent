"""Configuration and constants for Claude Agent server."""
...
def _normalize_storage_namespace(value, default='claude'):
    ...
    for char in token:
        if char.isalnum():
            if pending_separator and normalized_chars:
...
def _storage_filename(default_name, namespaced_suffix):
    if MODEL_STORAGE_NAMESPACE == 'claude':
        return default_name
    return f'{MODEL_STORAGE_NAMESPACE}_{namespaced_suffix}.json'


MODEL_STORAGE_NAMESPACE = _normalize_storage_namespace(
...
MODEL_DEFAULT_MODEL = MODEL_PROVIDER_DEFAULT_MODELS.get(MODEL_DEFAULT_PROVIDER, MODEL_CLAUDE_DEFAULT_MODEL)
...
SECRET_KEY = os.environ.get('MODEL_CHAT_SECRET_KEY', 'claude-chat-secret-key-change-in-production')
