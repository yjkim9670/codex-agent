...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (workModeFolderContextBtn) {
        workModeFolderContextBtn.addEventListener('click', () => {
            addCurrentDirectoryToChatContext(FILE_PANEL_VARIANT_WORK_MODE);
        });
    }
    ...
    if (fileBrowserFolderContextBtn) {
        fileBrowserFolderContextBtn.addEventListener('click', () => {
            addCurrentDirectoryToChatContext(FILE_PANEL_VARIANT_OVERLAY);
        });
    }
...
function getWorkModeFileElements() {
    ...
    return {
        ...
        folderContextBtn: document.getElementById('codex-work-mode-folder-context'),
...
function isLikelyVirtualKeyboardEnvironment() {
    ...
    if (/\bMac/i.test(platform) && touchPoints > 1) {
        return true;
    }
...
function getVisualViewportMetrics() {
    ...
    const isPinchZoomed = Number.isFinite(scale) && scale > 1.01;
    const visualHeight = isPinchZoomed ? NaN : Number(viewport?.height);
    const offsetTop = isPinchZoomed ? 0 : Math.max(0, Number(viewport?.offsetTop) || 0);
...
function getUsableMobileViewportHeight(metrics = getVisualViewportMetrics()) {
    const visualBottom = metrics.hasVisualHeight
        ...
        : NaN;
    const nextHeight = Number.isFinite(visualBottom) && visualBottom > 0
        ? (metrics.hasLayoutHeight ? Math.min(visualBottom, metrics.layoutHeight) : visualBottom)
...
function isMobileKeyboardOpen(isMobile = isMobileViewportBehaviorActive(), metrics = getVisualViewportMetrics()) {
    ...
    if (
        ...
        && lastStableMobileViewportHeight
        && isLikelyVirtualKeyboardEnvironment()
        && lastStableMobileViewportHeight - usableHeight > MOBILE_KEYBOARD_BASELINE_DELTA
    ) {
        return true;
    }
    if (metrics.hasVisualHeight && metrics.hasLayoutHeight) {
        const viewportDelta = metrics.layoutHeight - metrics.visualHeight;
        const bottomOcclusion = Number.isFinite(usableHeight)
            ? metrics.layoutHeight - usableHeight
            : 0;
        if (Math.max(viewportDelta, bottomOcclusion) > MOBILE_KEYBOARD_VIEWPORT_DELTA) {
            return true;
        }
        return false;
    }
...
function getPromptViewportBottom(metrics = getVisualViewportMetrics()) {
    if (!metrics.hasVisualHeight) {
        return metrics.hasLayoutHeight ? metrics.layoutHeight : NaN;
    }
    if (metrics.offsetTop <= 0) {
        return metrics.visualHeight;
    }
    const app = document.querySelector('.app');
    const appTop = Number(app?.getBoundingClientRect?.().top);
    if (Number.isFinite(appTop) && Math.abs(appTop) < Math.abs(appTop + metrics.offsetTop)) {
        return metrics.visualHeight + metrics.offsetTop;
    }
    return metrics.visualHeight;
}
...
function applyMobilePromptLift({ isMobile = isMobileViewportBehaviorActive(), keyboardOpen = false, metrics = getVisualViewportMetrics() } = {}) {
    const root = document.documentElement;
    if (!root) return;
    ...
    if (isMobile && keyboardOpen && promptForm?.contains(document.activeElement)) {
        ...
        if (Number.isFinite(rect?.bottom) && Number.isFinite(viewportBottom)) {
            const currentLift = Number.isFinite(lastAppliedMobilePromptLift)
                ? lastAppliedMobilePromptLift
                : 0;
            const currentTranslateY = currentLift * -1;
            const unshiftedPromptBottom = rect.bottom - currentTranslateY;
            const targetGap = isAndroidPlatform()
...
function setupMobileViewportBehavior(mobileMedia, input) {
    ...
    const handleLayoutModeChange = event => {
        syncViewportState({ normalizeScroll: isMobileViewportBehaviorActive(Boolean(event?.matches)) });
    };
...
function setupMobileSettingsInputBehavior(mobileMedia, inputs) {
    ...
    const isKeyboardLayoutActive = () => isMobileViewportBehaviorActive(mobileMedia.matches);
    ...
    const clearFocusStateIfNeeded = () => {
        if (!isKeyboardLayoutActive() || hasFocusedField()) return;
        applyMobileFocusState(false);
        syncMobileKeyboardState(true);
    };

    const handleLayoutModeChange = event => {
        if (isMobileViewportBehaviorActive(Boolean(event?.matches))) return;
        applyMobileFocusState(false);
    ...
    fields.forEach(field => {
        field.addEventListener('focus', () => {
            if (!isKeyboardLayoutActive()) return;
            syncMobileKeyboardState(true);
            if (mobileMedia.matches) {
...
function getFileBrowserElements() {
    ...
    return {
        ...
        folderContextBtn: document.getElementById('codex-file-browser-folder-context'),
...
function setFileBrowserDirectoryLoading(isLoading, message = '디렉터리 목록을 불러오는 중...') {
    ...
    syncFilePanelFolderContextButton(FILE_PANEL_VARIANT_OVERLAY, { loading });
}
...
function getFilePanelPreviewContextRoot(variant) {
    const normalizedVariant = normalizeFilePanelVariant(variant);
    const state = getFilePanelEditState(normalizedVariant);
    return normalizeFileBrowserRoot(state?.root || getFilePanelCurrentRoot(normalizedVariant));
}

function getFilePanelDirectoryContextPath(variant) {
    const normalizedVariant = normalizeFilePanelVariant(variant);
    const previewPath = getFilePanelPreviewContextPath(normalizedVariant);
    if (previewPath) {
        return getFileBrowserParentPath(previewPath);
    }
    return getFilePanelCurrentPath(normalizedVariant);
}

function getFilePanelDirectoryContextRoot(variant) {
    const normalizedVariant = normalizeFilePanelVariant(variant);
    const previewPath = getFilePanelPreviewContextPath(normalizedVariant);
    if (previewPath) {
        return getFilePanelPreviewContextRoot(normalizedVariant);
    }
    return getFilePanelCurrentRoot(normalizedVariant);
}
...
function buildFilePanelDirectoryChatContextText(root, directoryPath) {
    const normalizedDirectoryPath = normalizeFileBrowserRelativePath(directoryPath);
    return [
        '[Folder Context]',
        `folder: ${formatFileBrowserDisplayPath(root, normalizedDirectoryPath)}`
    ].join('\n');
}

function syncFilePanelFolderContextButton(variant, { loading = false } = {}) {
    const normalizedVariant = normalizeFilePanelVariant(variant);
    const config = getFilePanelVariantConfig(normalizedVariant);
    const elements = config.getElements();
    const button = elements?.folderContextBtn;
    if (!button) return;
    const root = getFilePanelDirectoryContextRoot(normalizedVariant);
    const directoryPath = getFilePanelDirectoryContextPath(normalizedVariant);
    const displayPath = formatFileBrowserDisplayPath(root, directoryPath);
    const label = `현재 폴더 위치를 채팅에 추가 (${displayPath})`;
    const isBusy = Boolean(loading)
        || isFilePanelLoading(elements)
        || isFilePanelBulkActionInFlight(normalizedVariant);
    button.disabled = isBusy;
    updateFilePanelActionButtonLabel(button, label);
    setIconButtonScreenReaderText(button, label);
    syncHoverTooltipFromLabel(button, label);
}
...
function syncFilePanelSelectionBar(variant) {
    ...
    if (elements.addContextBtn) {
        updateFilePanelActionButtonLabel(
            ...
            usingPreviewContext ? '미리보기 파일과 폴더 위치를 채팅에 넣기' : '선택 항목 경로를 채팅에 넣기'
        );
...
function buildFilePanelChatContextText(root, selectedPaths, { includePreviewDirectory = false } = {}) {
    const paths = Array.from(createNormalizedRelativePathSet(selectedPaths));
    if (!paths.length) return '';
    if (includePreviewDirectory && paths.length === 1) {
        return buildFilePanelPreviewChatContextText(root, paths[0]);
    }
    return paths
...
async function addSelectedFilesToChatContext(variant) {
    ...
    try {
        ...
        const contextText = await buildFilePanelChatContextText(
            contextRoot,
            contextPaths,
            { includePreviewDirectory: usingPreviewContext }
        );
        ...
        showToast(`${usingPreviewContext ? '미리보기 파일과 폴더 위치' : `선택 항목 경로 ${contextPaths.length}개`}를 채팅 입력에 넣었습니다.`, {
            tone: 'success',
            durationMs: 2600
        });
        return true;
...
async function addCurrentFileToChatContext(variant) {
    ...
    try {
        ...
        const contextText = buildFilePanelPreviewChatContextText(contextRoot, currentPath);
        if (!appendTextToChatInput(contextText)) {
            throw new Error('채팅 입력창을 찾을 수 없습니다.');
        }
        if (normalizedVariant === FILE_PANEL_VARIANT_WORK_MODE && isMobileLayout() && isWorkModeEnabled()) {
...
function addCurrentDirectoryToChatContext(variant) {
    const normalizedVariant = normalizeFilePanelVariant(variant);
    try {
        const contextRoot = getFilePanelDirectoryContextRoot(normalizedVariant);
        const directoryPath = getFilePanelDirectoryContextPath(normalizedVariant);
        const contextText = buildFilePanelDirectoryChatContextText(contextRoot, directoryPath);
        if (!appendTextToChatInput(contextText)) {
            throw new Error('채팅 입력창을 찾을 수 없습니다.');
        }
        if (normalizedVariant === FILE_PANEL_VARIANT_WORK_MODE && isMobileLayout() && isWorkModeEnabled()) {
        ...
        showToast('현재 폴더 위치를 채팅 입력에 넣었습니다.', {
            tone: 'success',
            durationMs: 2400
        });
        return true;
    } catch (error) {
        showToast(normalizeError(error, '폴더 위치를 채팅 입력에 넣지 못했습니다.'), {
            tone: 'error',
            durationMs: 4200
        });
        return false;
...
function setGitFileRevertButtonsDisabled(disabled, activeButton = null) {
    document.querySelectorAll('.git-file-tree-revert').forEach(button => {
...
async function handleGitFileRevert(button, path, repoTarget = gitSyncOverlayRepoTarget) {
    ...
    gitFileRevertingPath = normalizedPath;
    setGitFileRevertButtonsDisabled(true, button);
    ...
    try {
        ...
        gitMutationInFlight = false;
        gitFileRevertingPath = '';
        setGitButtonBusy(button, false);
        setGitFileRevertButtonsDisabled(false);
        void refreshGitBranchStatus({ force: true, updateOverlay: true });
    }
}
