...
from ..config import (
    GEMINI_MAX_MODEL_CHARS,
    GEMINI_MAX_PROMPT_CHARS,
    GEMINI_MAX_REASONING_CHARS,
    GEMINI_MAX_TITLE_CHARS,
...
from ..services.gemini_chat import (
    ...
    build_gemini_prompt,
    create_session,
    cleanup_gemini_streams,
    delete_session,
    ensure_default_title,
    execute_gemini_prompt,
    finalize_gemini_stream,
    ...
    list_sessions,
    rename_session,
    start_gemini_stream_for_session,
    update_settings,
    stop_gemini_stream,
)
...
@bp.route('/api/gemini/usage')
def gemini_usage():
    return jsonify({'usage': get_usage_summary()})


@bp.route('/api/gemini/settings', methods=['PATCH'])
def gemini_settings_update():
    payload = request.get_json(silent=True) or {}
    model = payload.get('model')
    reasoning = payload.get('reasoning_effort')
    if model is not None:
        model = str(model).strip()
        if len(model) > GEMINI_MAX_MODEL_CHARS:
            return jsonify({'error': '모델 이름이 너무 깁니다.'}), 400
    if reasoning is not None:
        reasoning = str(reasoning).strip()
        if len(reasoning) > GEMINI_MAX_REASONING_CHARS:
            return jsonify({'error': 'approval mode 값이 너무 깁니다.'}), 400
...
@bp.route('/api/gemini/sessions')
def gemini_sessions():
    return jsonify({'sessions': list_sessions()})


@bp.route('/api/gemini/sessions', methods=['POST'])
def gemini_sessions_create():
    payload = request.get_json(silent=True) or {}
...
@bp.route('/api/gemini/sessions/<session_id>')
def gemini_session_detail(session_id):
...
@bp.route('/api/gemini/sessions/<session_id>', methods=['PATCH'])
def gemini_session_rename(session_id):
    payload = request.get_json(silent=True) or {}
    title = (payload.get('title') or '').strip()

    if not title:
        return jsonify({'error': '세션 이름이 비어 있습니다.'}), 400
    if len(title) > GEMINI_MAX_TITLE_CHARS:
...
@bp.route('/api/gemini/sessions/<session_id>', methods=['DELETE'])
def gemini_session_delete(session_id):
    active_stream_id = get_active_stream_id_for_session(session_id)
    if active_stream_id:
        return jsonify({
...
@bp.route('/api/gemini/sessions/<session_id>/message', methods=['POST'])
def gemini_session_message(session_id):
    payload = request.get_json(silent=True) or {}
    prompt = (payload.get('prompt') or '').strip()

    if not prompt:
        return jsonify({'error': '프롬프트가 비어 있습니다.'}), 400
    if len(prompt) > GEMINI_MAX_PROMPT_CHARS:
    ...
    prompt_with_context = build_gemini_prompt(session.get('messages', []), prompt)
    user_message = append_message(session_id, 'user', prompt)
    if not user_message:
        return jsonify({'error': '메시지를 저장하지 못했습니다.'}), 500

    started_at = time.time()
    output, error = execute_gemini_prompt(prompt_with_context)
...
@bp.route('/api/gemini/sessions/<session_id>/message/stream', methods=['POST'])
def gemini_session_message_stream(session_id):
    payload = request.get_json(silent=True) or {}
    prompt = (payload.get('prompt') or '').strip()
    ...
    ensure_default_title(session_id, prompt)
    prompt_with_context = build_gemini_prompt(session.get('messages', []), prompt)
    start_result = start_gemini_stream_for_session(session_id, prompt, prompt_with_context)
...
@bp.route('/api/gemini/streams/<stream_id>')
def gemini_stream_output(stream_id):
    cleanup_gemini_streams()
    try:
    ...
    data = read_gemini_stream(stream_id, output_offset, error_offset)
    if not data:
        return jsonify({'error': '스트림을 찾을 수 없습니다.'}), 404

    saved_message = None
    if data.get('done') and not data.get('saved'):
        saved_message = finalize_gemini_stream(stream_id)
        data = read_gemini_stream(stream_id, output_offset, error_offset)
        if data:
            data['saved'] = True
...
@bp.route('/api/gemini/streams')
def gemini_streams_list():
    cleanup_gemini_streams()
    include_done = request.args.get('include_done') == '1'
    return jsonify({'streams': list_gemini_streams(include_done=include_done)})


@bp.route('/api/gemini/streams/<stream_id>/stop', methods=['POST'])
def gemini_stream_stop(stream_id):
    result = stop_gemini_stream(stream_id)
    if not result:
        return jsonify({'error': '스트림을 찾을 수 없습니다.'}), 404
    return jsonify(result)


@bp.route('/api/gemini/git/<action>', methods=['POST', 'GET'])
def gemini_git_action(action):
    payload = request.get_json(silent=True) or {}
    if request.method == 'GET':
        if action == 'push':
            ...
            payload = {'repo_target': request.args.get('repo_target') or 'gemini_agent'}
        else:
    ...
    if action == 'sync' and not str(payload.get('repo_target') or '').strip():
        # Keep top-right sync pinned to gemini_agent even if legacy clients omit payload.
        payload['repo_target'] = 'gemini_agent'
