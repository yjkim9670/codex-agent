"""Gemini chat session storage and execution helpers."""
...
from .. import state
from ..config import (
    GEMINI_CHAT_STORE_PATH,
    GEMINI_CONFIG_PATH,
    GEMINI_CONTEXT_MAX_CHARS,
    GEMINI_EXEC_TIMEOUT_SECONDS,
    GEMINI_SESSIONS_PATH,
    GEMINI_SETTINGS_PATH,
    GEMINI_STREAM_TTL_SECONDS,
    WORKSPACE_DIR,
)
...
_GEMINI_AUTH_PATH = Path.home() / '.gemini' / 'auth.json'
...
def _load_data():
    if not GEMINI_CHAT_STORE_PATH.exists():
        return {'sessions': []}
    try:
        data = json.loads(GEMINI_CHAT_STORE_PATH.read_text(encoding='utf-8'))
...
def _save_data(data):
    GEMINI_CHAT_STORE_PATH.parent.mkdir(parents=True, exist_ok=True)
    GEMINI_CHAT_STORE_PATH.write_text(
...
def _read_gemini_config_text():
    try:
        return GEMINI_CONFIG_PATH.read_text(encoding='utf-8')
...
def _read_workspace_settings():
    try:
        raw = GEMINI_SETTINGS_PATH.read_text(encoding='utf-8')
    except FileNotFoundError:
        return {}
    except Exception:
        return {}
...
def update_settings(model=None, reasoning_effort=None):
    with _CONFIG_LOCK:
        ...
        if not current and not GEMINI_SETTINGS_PATH.exists():
            text = _read_gemini_config_text()
    ...
    try:
        with path.open('r', encoding='utf-8') as file_handle:
            for line in file_handle:
                ...
                is_gemini_limit = limit_id == 'gemini'
                is_model_scoped = bool(limit_id) and limit_id.startswith('gemini_')
                if is_gemini_limit:
                    quality = 4
...
def _read_account_name():
    try:
        raw = _GEMINI_AUTH_PATH.read_text(encoding='utf-8')
...
def get_usage_summary():
    ...
    if not GEMINI_SESSIONS_PATH.exists():
        return {'five_hour': None, 'weekly': None, 'account_name': account_name}
    try:
        files = sorted(
            GEMINI_SESSIONS_PATH.rglob('*.jsonl'),
...
def _compose_structured_prompt(memory_lines, recent_blocks, prompt_text):
    sections = [
        (
            'You are Gemini CLI running inside a coding workspace.\n'
...
def build_gemini_prompt(messages, prompt):
    if not isinstance(messages, list):
        messages = []

    max_chars = max(1200, int(GEMINI_CONTEXT_MAX_CHARS))
...
def _normalize_approval_mode(value):
    ...
    if mode in ('default', 'auto_edit', 'yolo'):
        return mode
    # Backward-compatible mapping for copied Codex settings.
    legacy_map = {
        'low': 'default',
        'medium': 'default',
        'high': 'auto_edit',
...
def _build_gemini_command(prompt):
    cmd = ['gemini']
    settings = get_settings()
    model = settings.get('model')
    if model:
        cmd.extend(['--model', model])
    approval_mode = _normalize_approval_mode(settings.get('reasoning_effort'))
    cmd.extend(['--approval-mode', approval_mode])
    cmd.append(prompt)
    return cmd


def execute_gemini_prompt(prompt):
    WORKSPACE_DIR.mkdir(parents=True, exist_ok=True)
    cmd = _build_gemini_command(prompt)
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=GEMINI_EXEC_TIMEOUT_SECONDS,
            check=False
        )
    except FileNotFoundError:
        return None, 'gemini 명령을 찾을 수 없습니다.'
    except subprocess.TimeoutExpired:
        return None, 'Gemini 응답 시간이 초과되었습니다.'
    except Exception as exc:
        return None, f'Gemini 실행 중 오류가 발생했습니다: {exc}'

    output_text = (result.stdout or '').strip()

    if result.returncode != 0:
        error_text = (result.stderr or '').strip()
        return None, error_text or output_text or 'Gemini 실행에 실패했습니다.'
...
def _append_stream_chunk(stream_id, key, chunk):
    if not chunk:
        return
    with state.gemini_streams_lock:
        stream = state.gemini_streams.get(stream_id)
        if not stream:
            return
...
def _run_gemini_stream(stream_id, prompt):
    cmd = _build_gemini_command(prompt)
    try:
        ...
        _append_stream_chunk(stream_id, 'error', 'gemini 명령을 찾을 수 없습니다.\n')
        with state.gemini_streams_lock:
            stream = state.gemini_streams.get(stream_id)
            if stream:
                stream['done'] = True
        ...
        _append_stream_chunk(stream_id, 'error', f'Gemini 실행 중 오류가 발생했습니다: {exc}\n')
        with state.gemini_streams_lock:
            stream = state.gemini_streams.get(stream_id)
            if stream:
                stream['done'] = True
    ...
    with state.gemini_streams_lock:
        ...
        if stream:
            ...
            stream['updated_at'] = time.time()
            stream['process'] = None
    finalize_gemini_stream(stream_id)


def create_gemini_stream(session_id, prompt):
    ...
    with state.gemini_streams_lock:
        state.gemini_streams[stream_id] = stream

    thread = threading.Thread(
        target=_run_gemini_stream,
...
def _find_active_stream_id_locked(session_id):
    for stream_id, stream in state.gemini_streams.items():
...
def start_gemini_stream_for_session(session_id, prompt, prompt_with_context):
    ...
    with submit_lock:
        ...
        stream_id = create_gemini_stream(session_id, prompt_with_context)
        return {
            'ok': True,
            'stream_id': stream_id,
...
def get_gemini_stream(stream_id):
    with state.gemini_streams_lock:
        stream = state.gemini_streams.get(stream_id)
        return deepcopy(stream) if stream else None


def list_gemini_streams(include_done=False):
    streams = []
    with state.gemini_streams_lock:
        for stream in state.gemini_streams.values():
...
def read_gemini_stream(stream_id, output_offset=0, error_offset=0):
    with state.gemini_streams_lock:
        stream = state.gemini_streams.get(stream_id)
        if not stream:
            return None
...
def finalize_gemini_stream(stream_id):
    with state.gemini_streams_lock:
        stream = state.gemini_streams.get(stream_id)
    ...
    message_text = error or output or 'Gemini 실행에 실패했습니다.'
    return append_message(session_id, 'error', message_text, metadata)


def stop_gemini_stream(stream_id):
    with state.gemini_streams_lock:
        stream = state.gemini_streams.get(stream_id)
        if not stream:
            return None
    ...
    with state.gemini_streams_lock:
        for stream_id, stream in state.gemini_streams.items():
            if not stream.get('done'):
                continue
            if now - stream.get('updated_at', now) > GEMINI_STREAM_TTL_SECONDS:
                stale_ids.append(stream_id)
        for stream_id in stale_ids:
            state.gemini_streams.pop(stream_id, None)
