...
function updateStatusDisplay() {
    const status = document.getElementById('gemini-chat-status');
    if (!status) return;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const input = document.getElementById('gemini-chat-input');
    const newSessionBtn = document.getElementById('gemini-chat-new-session');
    const refreshBtn = document.getElementById('gemini-chat-refresh');
    const chatFullscreenBtn = document.getElementById('gemini-chat-fullscreen');
    const messages = document.getElementById('gemini-chat-messages');
    const scrollToLatestBtn = document.getElementById('gemini-chat-scroll-bottom');
    const streamMonitorCloseBtn = document.getElementById('gemini-stream-monitor-close');
    const streamMonitorToggle = document.getElementById('gemini-stream-monitor-toggle');
    const streamMonitor = document.getElementById('gemini-stream-monitor');
    const sessionsPanel = document.querySelector('.sessions');
    const sessionsToggle = document.getElementById('gemini-sessions-toggle');
    ...
    const gitCommitBtn = document.getElementById('gemini-git-commit');
    const gitPushBtn = document.getElementById('gemini-git-push');
    const gitSyncBtn = document.getElementById('gemini-git-sync');
    const branchOverlayCommitBtn = document.getElementById('gemini-branch-overlay-commit');
    const branchOverlayPushBtn = document.getElementById('gemini-branch-overlay-push');
    const branchOverlayStageAllBtn = document.getElementById('gemini-branch-overlay-stage-all');
    const branchOverlayStageNoneBtn = document.getElementById('gemini-branch-overlay-stage-none');
    const branchOverlayCommitMessageInput = document.getElementById('gemini-branch-overlay-commit-message');
    const syncOverlayFetchBtn = document.getElementById('gemini-sync-overlay-fetch');
    const syncOverlaySyncBtn = document.getElementById('gemini-sync-overlay-sync');
    const syncOverlayCommitBtn = document.getElementById('gemini-sync-overlay-commit');
    const syncOverlayPushBtn = document.getElementById('gemini-sync-overlay-push');
    const syncOverlayRefreshBtn = document.getElementById('gemini-sync-overlay-refresh');
    const syncOverlayTargetButtons = Array.from(
        document.querySelectorAll('#gemini-sync-overlay .sync-overlay-target[data-repo-target]')
    );
    ...
    const branchOverlay = document.getElementById('gemini-branch-overlay');
    const branchOverlayClose = document.getElementById('gemini-branch-overlay-close');
    const branchOverlayCloseFooter = document.getElementById('gemini-branch-overlay-close-footer');
    const branchOverlayRefresh = document.getElementById('gemini-branch-overlay-refresh');
    if (branchOverlay) {
    ...
    const syncOverlay = document.getElementById('gemini-sync-overlay');
    const syncOverlayClose = document.getElementById('gemini-sync-overlay-close');
    const syncOverlayCloseFooter = document.getElementById('gemini-sync-overlay-close-footer');
    if (syncOverlay) {
...
function initializeLiveWeatherPanel(mobileMedia) {
    const panel = document.getElementById('gemini-live-weather-panel');
    const toggle = document.getElementById('gemini-live-weather-toggle');
    const permissionToggle = document.getElementById('gemini-weather-permission');
    if (!panel || !toggle || !permissionToggle) return;

    initializeHoverTooltipInteractions();
    syncLiveWeatherLayout(Boolean(mobileMedia?.matches));
    const serverDirectory = document.getElementById('gemini-server-directory');
    const serverDirectoryPath = document.getElementById('gemini-server-directory-path');
...
function updateLiveWeatherPanelTitle() {
    const panel = document.getElementById('gemini-live-weather-panel');
    const title = document.getElementById('gemini-live-weather-title');
...
function updateLiveDatetime() {
    const datetime = document.getElementById('gemini-live-datetime');
    if (!datetime) return;
...
function syncWeatherCurrentRowTooltip(currentElement = null) {
    const valueElement = currentElement || document.getElementById('gemini-weather-current');
...
function updateThemeSwitchTooltip(themeToggle = null) {
    const toggle = themeToggle || document.getElementById('gemini-theme-toggle');
    if (!toggle) return;
...
async function loadLiveWeatherData({ silent = false, positionOverride = null } = {}) {
    ...
    const todayElement = document.getElementById('gemini-weather-today');
    const tomorrowElement = document.getElementById('gemini-weather-tomorrow');
...
function syncSidebarStackLayout() {
    ...
    if (!stack) return;
    const weatherPanel = document.getElementById('gemini-live-weather-panel');
    const sessionsPanel = document.querySelector('.sessions');
...
function setChatFullscreen(enabled) {
    const app = document.querySelector('.app');
    const button = document.getElementById('gemini-chat-fullscreen');
...
function setupMobileSettingsInputBehavior(mobileMedia, inputs) {
    ...
    const keepFieldVisible = (field, behavior = 'auto') => {
        if (!field) return;
        const controlsBody = document.getElementById('gemini-controls-body');
...
function setControlsCollapsed(collapsed, { persist = true } = {}) {
    const controls = document.getElementById('gemini-controls');
    const toggle = document.getElementById('gemini-controls-toggle');
...
function applyTheme(theme, { persist = true } = {}) {
    ...
    const toggle = document.getElementById('gemini-theme-toggle');
    if (toggle) {
...
function getStreamMonitorElements() {
    const container = document.getElementById('gemini-stream-monitor');
    if (!container) return null;
    return {
        container,
        list: document.getElementById('gemini-stream-monitor-list'),
        empty: document.getElementById('gemini-stream-monitor-empty'),
        output: document.getElementById('gemini-stream-monitor-output'),
        outputTitle: document.getElementById('gemini-stream-monitor-title'),
        outputStatus: document.getElementById('gemini-stream-monitor-status'),
        outputContent: document.getElementById('gemini-stream-monitor-content')
    };
}

function setStreamMonitorCollapsed(collapsed) {
    const monitor = document.getElementById('gemini-stream-monitor');
    if (!monitor) return;
    monitor.classList.toggle('is-collapsed', Boolean(collapsed));
    const toggle = document.getElementById('gemini-stream-monitor-toggle');
    if (toggle) {
        toggle.setAttribute('aria-expanded', String(!collapsed));
    }
}
...
function getGitBranchOverlayElements() {
    ...
    return {
        overlay,
        subtitle: document.getElementById('gemini-branch-overlay-subtitle'),
        meta: document.getElementById('gemini-branch-overlay-meta'),
        selection: document.getElementById('gemini-branch-overlay-selection'),
        stageAllBtn: document.getElementById('gemini-branch-overlay-stage-all'),
        stageNoneBtn: document.getElementById('gemini-branch-overlay-stage-none'),
        commitMessage: document.getElementById('gemini-branch-overlay-commit-message'),
        commitBtn: document.getElementById('gemini-branch-overlay-commit'),
        pushBtn: document.getElementById('gemini-branch-overlay-push'),
        loading: document.getElementById('gemini-branch-overlay-loading'),
        empty: document.getElementById('gemini-branch-overlay-empty'),
        list: document.getElementById('gemini-branch-overlay-list')
    };
}
...
function renderGitBranchOverlay(status) {
    const elements = getGitBranchOverlayElements();
    if (!elements) return;
    const branchElement = document.getElementById('gemini-git-branch');
...
function getGitSyncOverlayElements() {
    const overlay = document.getElementById('gemini-sync-overlay');
    if (!overlay) return null;
    return {
        overlay,
        subtitle: document.getElementById('gemini-sync-overlay-subtitle'),
        meta: document.getElementById('gemini-sync-overlay-meta'),
        targetButtons: Array.from(overlay.querySelectorAll('.sync-overlay-target[data-repo-target]')),
        fetchBtn: document.getElementById('gemini-sync-overlay-fetch'),
        syncBtn: document.getElementById('gemini-sync-overlay-sync'),
        commitBtn: document.getElementById('gemini-sync-overlay-commit'),
        pushBtn: document.getElementById('gemini-sync-overlay-push'),
        refreshBtn: document.getElementById('gemini-sync-overlay-refresh'),
        loading: document.getElementById('gemini-sync-overlay-loading'),
        empty: document.getElementById('gemini-sync-overlay-empty'),
        list: document.getElementById('gemini-sync-overlay-list')
    };
}
...
function getGitPushButtons(preferredButton = null) {
    const candidates = [
        preferredButton,
        document.getElementById('gemini-git-push'),
        document.getElementById('gemini-branch-overlay-push'),
        document.getElementById('gemini-sync-overlay-push')
    ];
    const seen = new Set();
...
function renderSessions() {
    const list = document.getElementById('gemini-session-list');
    if (!list) return;
    list.innerHTML = '';
    updateSessionsHeaderSummary();
...
function updateHeader(session) {
    const title = document.getElementById('gemini-chat-title');
    const meta = document.getElementById('gemini-chat-meta');
...
function setScrollToBottomButtonVisible(isVisible) {
    const button = document.getElementById('gemini-chat-scroll-bottom');
    if (!button) return;
...
function updateSessionsHeaderSummary() {
    const titleElement = document.getElementById('gemini-sessions-title');
    const sessionsPanel = document.querySelector('.sessions');
