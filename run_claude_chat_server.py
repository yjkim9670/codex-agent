...
def ensure_workspace_directory(script_path):
    ...
    _print_info(f"[INFO] Parent workspace enabled by default: {parent_dir}")
...
def apply_runtime_environment(config, config_path):
    ...
    if runtime_config.get('default_model') is not None:
        normalized_providers['claude'].setdefault('default_model', runtime_config.get('default_model'))
    legacy_model_options = _coerce_list(runtime_config.get('model_options'))
    if legacy_model_options and 'model_options' not in normalized_providers['claude']:
        normalized_providers['claude']['model_options'] = legacy_model_options
...
def parse_args(server_defaults, description='Run Claude Agent server.'):
...
def main(
    ...
    try:
        ...
        _print_error(f"[ERROR] Failed to import Claude Agent modules: {exc}")
