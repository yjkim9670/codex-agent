...
if ((show_help == 1)); then
    ...
    exec "${PYTHON_BIN}" "${SCRIPT_DIR}/run_claude_chat_server.py" "${original_args[@]}"
fi
...
exec "${PYTHON_BIN}" "${SCRIPT_DIR}/run_claude_chat_server.py" "${passthrough_args[@]}" --port "${final_port}"
