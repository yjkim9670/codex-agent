...
class GuiApp:
    ...
    def _on_error(self, title, message):
        ...
        # A 407 response is generated by a corporate proxy, not by the Git
        # repository. Check it before the general credential matcher so users
        # are not sent to configure an SSH key for a proxy sign-in issue.
        if is_proxy_authentication_error(message):
            self.status_var.set("Proxy authentication is required")
            logging.warning("Proxy authentication error detected: %s", message)
            user_message = (
                "The network proxy requires authentication.\n\n"
                "Complete the proxy/VPN sign-in, then retry Start Sync. "
                "If this persists, ask your network administrator to verify "
                "the proxy configuration for GitHub access.\n\n"
                f"Details:\n{message}"
            )
            self._show_diagnostic_dialog("Proxy Authentication Required", user_message)

        # Check for credential errors.
        elif is_credential_error(message):
            self.status_var.set("Authentication failed: credentials required")
            logging.warning("Credential error detected: %s", message)

            user_message = (
                "Git authentication was unavailable for this non-interactive sync.\n\n"
                "This can be temporary when Git Credential Manager is refreshing "
                "or saving a token, which explains why a later retry may succeed.\n\n"
                "To resolve a recurring issue, configure either an SSH key or a "
                "Personal Access Token in your Git credential manager.\n\n"
                f"Details:\n{message}"
            )
            self._show_diagnostic_dialog("Git Authentication Required", user_message)

        # Check for network errors.
        elif self._is_network_error(message):
            self.status_var.set("Waiting: network issue detected")
            logging.warning("Network error detected: %s", message)
            ...
            self._show_diagnostic_dialog("Network Connection Issue", user_message)
        else:
            # Handle general errors.
            self.status_var.set(f"Error: {title}")
            logging.error("%s: %s", title, message)
            if len(message) > 1200 or message.count("\n") > 16:
                self._show_diagnostic_dialog(title, message)
            else:
                messagebox.showerror(title, message)

    def _show_diagnostic_dialog(self, title: str, message: str) -> None:
        """Show complete diagnostics in a bounded, scrollable dialog."""
        ...
        height = min(620, max(340, screen_height - 180))
        dialog.geometry(f"860x{height}")
...
def is_proxy_authentication_error(error_msg: str) -> bool:
    """Return whether the failure is a proxy sign-in response (HTTP 407)."""
    error_lower = error_msg.lower()
    ...
    return any(indicator in error_lower for indicator in proxy_auth_indicators)
